...
require_literal "$pulse_dockerfile" 'FROM --platform=$BUILDPLATFORM mcr.microsoft.com/dotnet/sdk:' 'Pulse Dockerfile must pin .NET SDK stage to BUILDPLATFORM'
require_literal "$pulse_dockerfile" 'ARG TARGETARCH' 'Pulse Dockerfile must declare TARGETARCH'
require_literal "$pulse_dockerfile" 'linux-arm64' 'Pulse Dockerfile must map arm64 to a .NET runtime identifier'
require_literal "$pulse_dockerfile" 'linux-x64' 'Pulse Dockerfile must map amd64 to a .NET runtime identifier'
require_literal "$pulse_dockerfile" '--mount=type=cache' 'Pulse Dockerfile must use BuildKit cache mounts'
...
require_literal '.gitea/workflows/ci.yml' 'nuget-pulse-' 'CI workflow must cache Pulse NuGet packages'
